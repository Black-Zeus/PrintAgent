"""Módulo: Ticket de Retiro de Efectivo."""
import logging

import config_manager
from modules.escpos_helpers import get_cols, separator
from modules.image_renderer import PIL_AVAILABLE, render_retiro

logger = logging.getLogger("agent.ticket_retiro")
...
def render(printer, payload: dict, template: dict) -> None:
    ...
    if PIL_AVAILABLE:
        try:
            img = render_retiro(payload, template, font_name=font_name, font_size=font_size)
            if img is not None:
                printer.image(img); printer.text("\n"); printer.cut(); return
        except Exception as exc:
            logger.warning("Fallo TrueType en retiro, usando ESC/POS: %s", exc)

    cols = get_cols(template.get("paper_width_mm", 80))
    printer.set(align="center", bold=True)
    printer.text("--- RETIRO DE EFECTIVO ---\n")
    ...
    printer.text(f"Recibe:   {payload.get('receiver_name', '—')}\n")
    printer.text(separator(width=cols) + "\n")
