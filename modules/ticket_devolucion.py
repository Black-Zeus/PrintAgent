"""
Módulo: Ticket de Devolución
Renderizado PIL/TrueType — respeta el template configurado.
"""
import logging
...
from modules.image_renderer import PIL_AVAILABLE, render_devolucion

logger = logging.getLogger("agent.ticket_devolucion")
...
def _escpos_fallback(printer, payload: dict, template: dict) -> None:
    ...
    doc_type = truncate(payload.get("document_type", "TICKET DE DEVOLUCIÓN"), cols)
    ...
    print_items(printer, {"items": payload.get("return_items", [])}, body_cfg, cols=cols, font="b")
...
def render(printer, payload: dict, template: dict) -> None:
    ...
    if PIL_AVAILABLE:
        try:
            img = render_devolucion(payload, template, font_name=font_name, font_size=font_size)
            if img is not None:
                printer.image(img)
                printer.text("\n")
                printer.cut()
            ...
            logger.warning("Fallo renderizado TrueType en devolución, usando ESC/POS: %s", exc)
