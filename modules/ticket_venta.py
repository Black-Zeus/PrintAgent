...
Renderiza un ticket de venta estándar en impresora térmica.

Estrategia (en orden):
  1. Imagen TrueType via PIL (fuente configurable, tamaño configurable)
  2. Fallback ESC/POS bitmap si PIL no está disponible o falla
"""
import logging
...
from modules.image_renderer import PIL_AVAILABLE, render_venta

logger = logging.getLogger("agent.ticket_venta")


def _escpos_fallback(printer, payload: dict, template: dict) -> None:
    """Ruta ESC/POS clásica — se usa si PIL no está o falla."""
    ...
    doc_type = truncate(payload.get("document_type", "TICKET DE VENTA"), cols)
...
def render(printer, payload: dict, template: dict) -> None:
    ...
    if PIL_AVAILABLE:
        try:
            img = render_venta(payload, template, font_name=font_name, font_size=font_size)
            if img is not None:
                # Imagen TrueType: ticket completo incluyendo barcode
                printer.image(img)
                printer.text("\n")
                printer.cut()
                return
        except Exception as exc:
            logger.warning("Fallo renderizado TrueType, usando ESC/POS: %s", exc)
