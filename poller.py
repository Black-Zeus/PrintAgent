...
def _fetch_pending_jobs() -> list[dict]:
    try:
        resp = requests.get(
            f"{_server_url()}/print/agent/jobs/pending",
...
def _update_job_status(job_code: str, status: str, error: Optional[str] = None) -> None:
    ...
    try:
        requests.patch(
            f"{_server_url()}/print/agent/jobs/{job_code}/status",
            json=payload,
            headers=_api_headers(),
            timeout=5,
        )
...
def _execute_job(job: dict, template: dict) -> None:
...
def _poll_loop() -> None:
    ...
    state["running"] = True
    cycle = 0
    template = template_manager.get_current()

    while not _stop_event.is_set():
        # Verificar template periódicamente
        if cycle % _TEMPLATE_CHECK_INTERVAL == 0:
            updated, template = template_manager.check_and_update()
            if updated:
                logger.info("Template actualizado en ciclo %d", cycle)
...
def print_test() -> dict:
    """Imprime un ticket de prueba. Llamado desde el portal."""
    cfg = config_manager.load()
    printer_name = cfg.get("printer_name", "auto")
    p = printer_module.open_printer(printer_name)
    if p is None:
